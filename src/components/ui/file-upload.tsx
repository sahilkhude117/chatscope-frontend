...
export const FileUpload = ({
  ...
  const { getRootProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (error) => {
      console.log("File drop rejected:", error);
    },
  });
...
// --- GridPattern updated for black theme ---
        ...
        Array.from({ length: columns }).map((_, col) => {
          ...
          return (
            <div
              key={`${col}-${row}`}
              className={cn(
                "w-10 h-10 flex shrink-0 rounded-[2px] bg-neutral-950",
                index % 2 !== 0 && "shadow-[0px_0px_1px_3px_rgba(0,0,0,1)_inset]"
              )}
            />
          );
        })
